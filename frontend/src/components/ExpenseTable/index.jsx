...
const StyledDialog = muiStyled(Dialog)({
  "& .MuiDialog-paper": {
    backgroundColor: "rgb(0, 13, 5)",
    color: "white",
    border: "1px solidrgb(121, 105, 17)",
  },
});

const StyledDialogTitle = muiStyled(DialogTitle)({
  color: "white",
  borderBottom: "1px solid #ffd9009d",
});
...
const ExpenseTable = ({ onCreateExpense }) => {
  ...
  const handleNecessityChange = async (expenseId, newValue) => {
    try {
      await api.patch(`/api/expenses/${expenseId}/`, {
        necessity_level: newValue,
      });
      setExpenses(
        expenses.map((expense) =>
          expense.id === expenseId
  ...
  const getNecessityLabel = (value) => {
    return (
      NECESSITY_LEVELS.find((level) => level.value === value)?.label || value
    );
  };
  ...
  return (
    ...
    >
      ...
      >
        <StyledDialogTitle>Add New Expense</StyledDialogTitle>
        <DialogContent sx={{ backgroundColor: "rgb(0, 13, 5)", pt: 2 }}>
          <form
            ...
            <FormControl fullWidth>
              <StyledSelect
                ...
                onChange={(e) =>
                  setFormState({
                    ...formState,
                    necessity_level: e.target.value,
                  })
                }
                displayEmpty
              >
